...
echo"<form action='' method='post'>";
	...
	echo"<table class='ac'>";
 $f = fopen("/home/newton/recess.txt", "r") or exit("Unable to open file!");
    ...
    // Write links (get the data in the array)
       ...
       if (isset($_POST['accept'])) { 
         ...
         $loan="loan_request";
                 if($contr == $arrM[0]) {      
            $sq = "INSERT INTO contributions VALUES (NULL,'$arrM[1]','$arrM[2]','$arrM[3]','$arrM[4]','accepted')"; //here should be $arrM
           
            if (!mysqli_query($conn,$sq)) {
              die('Error: ' . mysqli_error());
            } 
            }else if($idea == $arrM[0]) {      
            $ql = "INSERT INTO ideas VALUES (NULL,'$arrM[1]','$arrM[2]','$arrM[3]','$arrM[4]','accepted')"; //here should be $arrM
            if (!mysqli_query($conn,$ql)) {
              die('Error: ' . mysqli_error());
            } 
            }else if($loan == $arrM[0]) {      
            $sl = "INSERT INTO loan_request VALUES (NULL,'$arrM[1]','$arrM[2]','$arrM[3]','accepted')"; //here should be $arrM
        ...
        $loan="loan_request";
                 if($contr == $arrM[0]) {      
            $sql = "INSERT INTO contributions VALUES (NULL,'$arrM[1]','$arrM[2]','$arrM[3]','$arrM[4]','denied')"; //here should be $arrM
            ...
            $sql = "INSERT INTO loan_request VALUES (NULL,'$arrM[1]','$arrM[2]','$arrM[3]','denied')"; //here should be $arrM
            if (!mysqli_query($conn,$sql)) {
              die('Error: ' . mysqli_error());
            } 
            }                                        
...
$a="SELECT idea,profit/loss FROM investmentIdeas WHERE moneytype='loss'";
$b=mysqli_query($conn,$a);
echo"<table border='1' cellspacing='0'style='margin-left:6%;color:black;text-align:center;font-size:10pt' >
