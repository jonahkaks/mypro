...
if(!(mysql_real_connect(conn,host,user,pass,dbname,port,unix_socket,flag))){
		...
		bzero(client,256);
read(sock,client,255);
bzero(password,256);
read(sock,password,255);


snprintf(statement, 256,"SELECT * FROM users WHERE name ='%s'  AND password='%s'",client,password);
...
res = mysql_use_result(conn);
	...
	send(sock,"Connected",10,0);
	 mysql_free_result(res);
	goto me;
	}else {
	
send(sock,"invalid",18,0);	
...
switch((int)*a * (int)*(a+1) * (int)*(a+2)){
		...
		case 1039896:
			...
			snprintf(statement, 256,"SELECT * FROM contributions WHERE name='%s'",client);
  ...
  while ((row = mysql_fetch_row(res)) != NULL){
	   ...
	   break;
		...
		case 1088780:
		snprintf(statement, 256,"SELECT * FROM benefits WHERE name='%s'",client);
  ...
  while ((row = mysql_fetch_row(res)) != NULL){
		...
		case 1162836:
		snprintf(statement, 256,"SELECT * FROM loans WHERE name ='%s'",client);
			 if (mysql_query(conn,statement)) {
      fprintf(stderr, "%s\n", mysql_error(conn));
